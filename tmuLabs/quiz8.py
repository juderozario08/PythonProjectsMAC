...
def remove_all_qs(s):
    return ''.join([i for i in s if i.lower() != 'q']).strip()

# Your third task will be to create a function called find_biggest_mult that takes in two nonempty lists of integers and returns the indices of the two numbers
# that result in the largest product (one from each list) that, when multiplied, give the biggest product. Return as a tuple.
...
# Your fourth task will be to create a recursive function rec_mul that takes in a list of integers L and if there is one element returns it, else it
# returns the value of the first integer in the list times rec_mul of the list minus that element
# TODO: complete rec_mul


def rec_mul(L):
    return L[0] if len(L) == 1 else L.pop(0)*rec_mul(L)
